import React from "react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogFooter,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";

const PreviewTeacher = ({ open, onClose, data = [] }) => {
  const teacherList = Array.isArray(data) ? data : [];
  console.log(teacherList);

  const hasErrors = teacherList.some((teacher) => !!teacher.error);
  return (
    <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
      <DialogContent className="!max-w-7xl  w-screen h-[90vh] rounded-lg p-6">
        {/* Thay max-w-full thành max-w-5xl */}
        <DialogHeader>
          <DialogTitle>Xem trước danh sách giảng viên</DialogTitle>
        </DialogHeader>
        <div className="h-[70vh] w-full overflow-auto mt-4 rounded-lg border">
          <div className="w-full overflow-x-auto">
            <table className="min-w-full text-sm border-collapse">
              <thead className="bg-gray-100 sticky top-0 z-10 shadow">
                <tr>
                  <th className="border p-2">STT</th>
                  <th className="border p-2">Mã GV</th>
                  <th className="border p-2">Họ</th>
                  <th className="border p-2">Tên</th>
                  <th className="border p-2">Email</th>
                  <th className="border p-2">Ngày sinh</th>
                  <th className="border p-2">Giới tính</th>
                  <th className="border p-2">Trạng thái</th>
                  {hasErrors && (
                    <th className="border p-2 text-red-600">Lỗi</th>
                  )}
                </tr>
              </thead>
              <tbody>
                {teacherList.map((teacher, idx) => (
                  <tr
                    key={idx}
                    className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}
                  >
                    <td className="border p-2">{teacher.stt}</td>
                    <td className="border p-2">{teacher.id}</td>
                    <td className="border p-2">{teacher.firstName}</td>
                    <td className="border p-2">{teacher.lastName}</td>
                    <td className="border p-2">{teacher.email}</td>
                    <td className="border p-2">{teacher.dateOfBirth}</td>
                    <td className="border p-2">{teacher.gender}</td>
                    <td className="border p-2">{teacher.status}</td>
                    {hasErrors && (
                      <td className="border p-2 text-red-600 italic">
                        {teacher.error || ""}
                      </td>
                    )}
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
        <DialogFooter className="mt-4">
          <Button onClick={onClose}>Đóng</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
};

export default PreviewTeacher;
